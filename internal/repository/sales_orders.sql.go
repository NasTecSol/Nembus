...
SELECT 
    so.id, so.organization_id, so.order_number, so.customer_id, so.store_id, so.order_date, so.delivery_date, so.status, so.subtotal, so.discount_amount, so.tax_amount, so.total_amount, so.price_list_id, so.created_by, so.metadata, so.created_at, so.updated_at,
