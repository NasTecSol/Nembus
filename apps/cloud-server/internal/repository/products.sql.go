...
const searchMasterProductCatalog = `-- name: SearchMasterProductCatalog :many
SELECT 
    product_id,
    sku,
    name,
...
WHERE organization_id = $1
  AND (
    ...
    COALESCE(category_name, '') ILIKE '%' || $2 || '%' OR
    COALESCE(category_code, '') ILIKE '%' || $2 || '%' OR
    COALESCE(brand_name, '') ILIKE '%' || $2 || '%' OR
    COALESCE(brand_code, '') ILIKE '%' || $2 || '%' OR
    barcodes::text ILIKE '%' || $2 || '%' OR
    variants::text ILIKE '%' || $2 || '%'
  )
ORDER BY product_id ASC
LIMIT $4 OFFSET $3
`

type SearchMasterProductCatalogParams struct {
	OrganizationID int32  `json:"organization_id"`
...
type SearchMasterProductCatalogRow struct {
	ProductID            int32            `json:"product_id"`
	Sku                  string           `json:"sku"`
	Name                 string           `json:"name"`
	Description          pgtype.Text      `json:"description"`
...
func (q *Queries) SearchMasterProductCatalog(ctx context.Context, arg SearchMasterProductCatalogParams) ([]SearchMasterProductCatalogRow, error) {
	rows, err := q.db.Query(ctx, searchMasterProductCatalog,
		arg.OrganizationID,
		arg.Search,
		arg.Offset,
		arg.Limit,
	...
	defer rows.Close()
	var items []SearchMasterProductCatalogRow
	for rows.Next() {
		var i SearchMasterProductCatalogRow
		if err := rows.Scan(
			&i.ProductID,
			&i.Sku,
			&i.Name,
...
const searchMasterProductCatalogCount = `-- name: SearchMasterProductCatalogCount :one
SELECT COUNT(*) 
FROM v_master_product_catalog
WHERE organization_id = $1
  AND (
...
type SearchMasterProductCatalogCountParams struct {
	OrganizationID int32  `json:"organization_id"`
	Search         string `json:"search"`
}

func (q *Queries) SearchMasterProductCatalogCount(ctx context.Context, arg SearchMasterProductCatalogCountParams) (int64, error) {
	row := q.db.QueryRow(ctx, searchMasterProductCatalogCount, arg.OrganizationID, arg.Search)
	var count int64
	err := row.Scan(&count)
	return count, err
}
