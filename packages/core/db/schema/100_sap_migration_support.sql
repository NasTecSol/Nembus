...
-- =====================================================================
--
-- OPERATIONAL NOTES (SAP → Nembus migration):
--
-- 1. DOMAIN ORDERING — ingest in this order (see apps/sap-agent engine.go
--    defaultDomainOrder): stores → users → uom/uom_groups → categories →
--   brands → products → barcodes → price_lists → inventory → payment_terms →
--   partners → bp_addresses → purchase_orders → goods_receipt_notes →
--   outgoing_payments → sales_orders → invoices → sales_returns →
--   transfer_requests → stock_movements → incoming_payments.
--   stock_movements resolves reference_id against documents ingested above.
--
-- 2. CURRENCY POLICY: SAR is the fallback everywhere (98_seed_currencies.sql);
--   a non-empty source DocCur wins when present.
--
-- 3. SINGLE-ORG KEY POLICY: SAP-derived business keys (PO-{DocNum},
--   INV-SAP-{DocNum}, CN-SAP-{DocNum}, RET-SAP-{DocNum}, TR-SAP-{DocNum},
--   GRN-{DocNum}, PAY-SAP-*, VPM-SAP-*, CAT-*, BRD-*, PL-*, PT-*, OCRD
--   CardCode, OUSR USER_CODE) are globally UNIQUE across organizations.
--   This migration is validated for ONE organization per Nembus org; a
--   second org migrating the same codes would collide. Add org-scoped
--   prefixes or org-scoped uniqueness before a multi-org migration.
--
-- 4. BALANCE AUTHORITY: business_partners.outstanding_balance and
--   customers.outstanding_balance snapshot SAP OCRD.Balance at migration
--   time (already reflecting historical payments, incl. on-account ones
--   recorded in customer_payments). Do not double-apply on-account payment
--   rows to the balance during migration.
--
-- 5. amount_paid: purchase_orders.metadata->>'amount_paid' is recomputed
--   from the vendor_payments ledger on each outgoing payment batch —
