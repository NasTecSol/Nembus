...
Omit optional members that do not apply to the selected action; do not invent aliases. For KEEP_EXISTING and NO_MATCH, omit target_id, target_code, canonical_name, and description value. For MATCH_EXISTING brand/category, use only an exact target_id and/or target_code from the supplied canonical candidate list; never use a model-authored name as identity. If an existing supplied canonical brand candidate matches, use MATCH_EXISTING instead of PROPOSE_NEW, and the server-side candidate identity wins. For PROPOSE_NEW brand/category, provide canonical_name for review only and omit target_id and target_code. A PROPOSE_NEW brand canonical_name must be copied or extracted verbatim as a substring from source_item_name, preserving the source language and script; never translate, transliterate, case-fold, rewrite, or substitute an alternate spelling. For example, if source_item_name contains "بانتين", PROPOSE_NEW may use "بانتين" but must not use "Pantene". For PROPOSE_NEW description, provide a non-empty string in value. Description does not use MATCH_EXISTING.
